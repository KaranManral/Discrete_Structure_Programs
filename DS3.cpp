...
class Relation
{
	private: char* a,*b;  //Arrays to store elements of two sets
			...
			int binSearch(char arr[], int l, int r, int x) //Function of Binary search which returns element postion if element is present
			{
    			while (l <= r)
				{
        			int m = l + (r - l) / 2;
	...
	public:	Relation()//Default Constructor
			...
			void createMatrix() //Function to create Relation Matrix
			{
				...
				{
					n=binSearch(a,0,size-1,r[i][0]);
					o=binSearch(b,0,size1-1,r[i][1]);
					if(n!=-1)
						if(o!=-1)
							m[n][o]=1;	
				}
